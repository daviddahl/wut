...
const MainUI = (configuration, storage, network) => {
  ...
  output = blessed.Log({
    ...
    parent: screen,
    scrollable: true,
    label: ` ${APP_TITLE}: Libp2p + Tweetnacl Chat `,
    top: 0,
    left: 0,
    width: '60%',
    height: '90%',
  ...
  input.on('keypress', (value, key) => {
    ...
    case RETURN:
      ...
      if (msg) {
        msg = msg.trim() // TRIM all input here instead of everywhere else :)
      }
      ...
      if (msg.indexOf('/') === 0) {
        ...
        output.log(`${APP_TITLE}: badly formed command: ${msg}`);
        break;
      }
  ...
  // nit: 'description' is spelt wrong ;)
  const HELP_COMMANDS = [
  ...
  const dm = (data) => {
    ...
    const ui = dmUI(screen, profile, storage, network);
