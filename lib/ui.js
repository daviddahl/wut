...
const e2eUI = (parent, profile, configuration, room) => {
  ...
  input.on('keypress', (value, key) => {
    ...
    case RETURN:
      ...
      let boxedDM = createDirectMessage(profile, plaintext, configuration);
      output.log(`${boxedDM}`);
      output.log(`${configuration.handle}: ${plaintext}`);
      output.log(profile.id);
      output.log(Array.isArray(profile.id));
      output.log(typeof profile.id);
      room.sendTo(profile.id, boxedDM);
      input.clearValue();
      break;
    case BACKSPACE:
